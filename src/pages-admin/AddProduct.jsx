  ...
  return (
    <div className="add-product-wrapper">
      ...
      <form className="add-product-form" onSubmit={handleSubmit}>
        ...
        <h3>Category</h3>
        <select name="category" value={product.category} onChange={handleChange} className="category-select" required>
        ...
        <div className="field-list">
          {sizes.map((size, index) => (
            <div className="field-row" key={index}>
              ...
              />
              {sizes.length > 1 && (
                <button type="button" className="btn-delete-field" onClick={() => removeSize(index)}>×</button>
              )}
            </div>
          ))}
        </div>
        <button type="button" className="btn-add-field" onClick={addSize}>+ Add Size</button>
        ...
        <div className="field-list">
          {colors.map((color, index) => (
            <div className="field-row" key={index}>
              ...
              />
              {colors.length > 1 && (
                <button type="button" className="btn-delete-field" onClick={() => removeColor(index)}>×</button>
              )}
            </div>
          ))}
        </div>
        <button type="button" className="btn-add-field" onClick={addColor}>+ Add Color</button>
